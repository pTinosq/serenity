...
1. **Tickers must come from the text.** Return a ticker when one of the following holds:
   ...
   If the text only refers to a sector, theme, or vague category — or to a company whose ticker is not well-known or not unambiguous — return null. Never invent, substitute, or guess a ticker.

2. **order_type is one of three values: BUY, SELL, NO_TRADE.**
   ...
   - **NO_TRADE** — the text does not give you enough to act on. Use NO_TRADE when:
     - No ticker can be extracted from the text. (In this case ticker must also be null.)
   ...
   When the sentiment is ambiguous, prefer NO_TRADE over guessing.

3. **Multi-ticker texts: pick the ticker with the highest conviction.** When several tickers appear, identify the one the text is primarily about — the one with the clearest directional thesis or the strongest sentiment behind it. Trade on that ticker. Only fall back to NO_TRADE when no single ticker stands out as the dominant, highest-conviction subject.

4. **Confidence reflects how clearly the text supports your chosen order_type.** Score 0.0 to 1.0. Strong, specific, factual statements score high. Hedged or mixed statements score low. For NO_TRADE, confidence is how sure you are that abstaining is correct — a clearly untradable text scores high; a borderline case scores low.
